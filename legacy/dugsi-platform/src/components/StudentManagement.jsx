...
const classes = [
  { id: 1, name: 'Fasal 5', level: 'Beginner', students: 15, teacher: 'Ustadha Khadija' },
  { id: 2, name: 'Fasal 6', level: 'Beginner', students: 18, teacher: 'Ustadh Muhammad' },
  { id: 3, name: 'Fasal 7', level: 'Intermediate', students: 12, teacher: 'Ustadha Fatima' },
  { id: 4, name: 'Fasal 8', level: 'Intermediate', students: 14, teacher: 'Ustadh Omar' },
  { id: 5, name: 'Fasal 9', level: 'Advanced', students: 10, teacher: 'Ustadh Abdullah' }
]
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Input
          ...
          value={formData.name}
          onChange={(e) => setFormData({...formData, name: e.target.value})}
          placeholder="e.g., Fasal 7A, Quran Class 1"
      ...
      <div>
        ...
        <select
          ...
          value={formData.level}
          onChange={(e) => setFormData({...formData, level: e.target.value})}
          className="w-full p-2 border border-input rounded-input bg-background"
          required
        >
          {levels.map(level => (
      ...
      <div className="flex gap-3 pt-4">
        <Button type="button" variant="outline" onClick={onCancel} className="flex-1 rounded-button">
          Cancel
        </Button>
        <Button type="submit" className="flex-1 rounded-button">
...
export function StudentManagement() {
  ...
  const handleExportStudents = () => {
    const csvData = students.map(student => ({
      ...
      'Name': student.name,
      'Arabic Name': student.arabicName,
      'Class': student.class,
      'Status': student.status,
      'Enrollment Date': student.enrollmentDate,
      'Parent Name': student.parentName,
      'Parent Phone': student.parentPhone,
      'Outstanding Fees': student.fees.outstanding
    }))
  ...
  const handleSaveClass = (classData) => {
    if (editingClass) {
      setClassesData(prev => prev.map(c => c.id === editingClass.id ? { ...classData, id: c.id } : c))
    } else {
      const newClass = {
        ...classData,
  ...
  const handleDeleteClass = (classData) => {
    if (confirm(`Are you sure you want to delete ${classData.name}?`)) {
      setClassesData(prev => prev.filter(c => c.id !== classData.id))
    }
  }
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <Button variant="outline" onClick={handleExportStudents} className="rounded-button">
            <Download className="w-4 h-4 mr-2" />
            Export
          </Button>
          <Button variant="outline" onClick={handleImportStudents} className="rounded-button">
        ...
        <TabsContent value="classes" className="space-y-6 mt-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-muted-foreground">Manage classes and view students</p>
            </div>
            <Button onClick={handleAddClass} className="rounded-button">
              ...
              <Card key={cls.id} className="rounded-card shadow-card">
                <CardContent className="p-6">
                  ...
                  <div className="mt-4 pt-4 border-t border-border space-y-2">
                    <Button variant="outline" onClick={() => handleViewClass(cls)} className="w-full rounded-button">
                      <Eye className="w-4 h-4 mr-2" />
                      View Students
                    </Button>
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" onClick={() => handleEditClass(cls)} className="flex-1 rounded-button">
                        <Edit className="w-4 h-4 mr-1" />
                        Edit
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => handleDeleteClass(cls)} className="flex-1 rounded-button text-destructive hover:text-destructive">
            ...
            <Card className="rounded-card shadow-card mt-6">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Class taught by {selectedClass.teacher}</CardDescription>
                  </div>
                  <Button variant="outline" onClick={() => setSelectedClass(null)} className="rounded-button">
                    Close
                  </Button>
                </div>
              </CardHeader>
              <CardContent>
                <div className="grid gap-4">
                  {students.filter(student => student.class === selectedClass.name).map(student => (
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button variant="outline" size="sm" onClick={() => handleViewStudent(student)} className="rounded-button">
                              <Eye className="w-4 h-4" />
                            </Button>
                          </div>
                        </div>
