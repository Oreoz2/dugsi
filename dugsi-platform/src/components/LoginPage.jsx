...
const PinInput = ({ value, onChange, disabled, error }) => {
      ...
      <div className="flex gap-2 justify-center">
        {[0, 1, 2, 3, 4, 5].map((index) => (
          <input
            ...
            onPaste={handlePaste}
            disabled={disabled}
            className={`w-12 h-12 text-center text-lg font-bold border rounded-modern focus-modern transition-modern ${
              error 
...
const OtpVerification = ({ phone, email, onVerify, onBack, loading }) => {
...
export function LoginPage({ onLogin, tenant }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Mock validation
      if (formData.username === 'admin' && formData.pin === '123456') {
        onLogin({
          username: formData.username,
          name: 'Ahmed Hassan',
          role: 'Admin',
          tenant: tenant?.name || 'Al-Noor Academy'
        })
      } else if (formData.username === 'teacher' && formData.pin === '654321') {
        onLogin({
          username: formData.username,
  ...
  const handleOtpVerify = async (otp) => {
    ...
    try {
      ...
      if (otp === '123456') {
        // In real app, redirect to PIN reset form
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-secondary/20 to-accent/10 flex items-center justify-center p-4">
      <div className="w-full max-w-md space-y-6 animate-fade-in">
        ...
        {/* Main Card */}
        <Card className="rounded-card shadow-modal border-0">
          ...
          <CardContent className="space-y-6">
            {error && (
              <Alert variant="destructive" className="rounded-modern">
            ...
            {isLocked && (
              <Alert className="rounded-modern border-orange-200 bg-orange-50">
        ...
        {/* Demo Credentials */}
        <Card className="rounded-card bg-muted/50 border-dashed">
          <CardContent className="p-4">
            <h4 className="font-medium text-sm mb-2">Demo Credentials:</h4>
            <div className="space-y-1 text-xs text-muted-foreground">
              <div>Admin: username: <code>admin</code>, PIN: <code>123456</code></div>
              <div>Teacher: username: <code>teacher</code>, PIN: <code>654321</code></div>
            </div>
          </CardContent>
        </Card>
      </div>
